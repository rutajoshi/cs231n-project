...
def video_process(video_file_path, dst_root_path, ext, fps=-1, size=240):
    if ext != video_file_path.suffix:
        print("Wrong extension: " + str(video_file_path.suffix))
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='',
        type=str,
        help='Dataset name (erus | mh | ntu | kinetics | mit | ucf101 | hmdb51 | activitynet)')
    parser.add_argument(
    ...
    if args.dataset in ['erus', 'mh', 'kinetics', 'mit', 'activitynet']:
        ext = '.mp4'
    else:
        ext = '.avi'
    ...
    if args.dataset == 'activitynet':
        ...
        args.n_jobs = 1
        print("Starting parallel jobs on: " + str(args.n_jobs) + " threads for " + str(len(class_dir_paths)) + " dir paths")
