...
def crop_and_sample(video_file_path, dst_root_path, keepframes=32):
    ...
    for i in range(1, 12):
        question_folder = "q" + str(i)
        question_folder_path = video_file_path / question_folder
        ctr_start = ctr
        last_face_seen = None
        if not os.path.isdir(question_folder_path):
            # Make 32 black frames and save them to the dst
            for j in range(keepframes):
                frame = Image.new('RGB', (427, 240))
                frame_name = image_name_formatter(ctr)
                frame.save(str(dst_dir_path) + "/" + frame_name)
                last_face_seen = frame
                ctr += 1

        else:
            # Copy 32 frames to dst
            ...
            if (num_frames < keepframes):
                ...
                for frame_file in all_frames:
                    ...
                    cropped_frame = frame_pil #frame_pil.crop((left, top, right, bottom)) #CROP THE FRAME USING ONLY FIRST FACE
                    frame_name = image_name_formatter(ctr)
